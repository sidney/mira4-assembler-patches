...
ostream & operator<<(ostream &ostr, AssemblyInfo & asi)
{
  FUNCSTART("friend ostream & AssemblyInfo::operator<<(ostream &ostr, const  &???)");
...
void AssemblyInfo::calcCurrentInfo()
{
  ...
  for(; csI != ASI_contigstats.end(); ++csI){
    ...
    if(coninfooffset==0                     // not taken yet
      ...
      bool avgcovfound=false;
      if(ASI_conf_large_mintotalcov>0
	 && csI->avg_coverage>= ASI_conf_large_mintotalcov*(2.0d/3)){
	avgcovfound=true;
      }else{
	for(uint32 st=0; st<ReadGroupLib::SEQTYPE_END; st++){
	  if(ASI_conf_large_minavgcov_perst[st]>0
	     && csI->avg_covperst[st] >= ASI_conf_large_minavgcov_perst[st]*(2.0d/3)){
	    avgcovfound=true;
	    break;
	  }
	}
  ...
  for(uint32 i=0; i<2; i++){
    ...
    if(!consizes[i].empty()){
      sort(consizes[i].begin(),consizes[i].end(), greater<uint32>());

      ASI_n50consize[i]=0;
      ASI_n90consize[i]=0;
      ASI_n95consize[i]=0;

      vector<uint32>::const_iterator dI=consizes[i].begin();
      uint32 totalconsize=0;
      for(; dI!=consizes[i].end(); dI++){
	totalconsize+=*dI;
    ...
    if(!conquals[i].empty()){
      sort(conquals[i].begin(),conquals[i].end(), greater<uint32>());
      ASI_avgconqual[i]=conquals[i][(conquals[i].end()-conquals[i].begin())/2];
    }
    if(!avgtotalcov[i].empty()){
      sort(avgtotalcov[i].begin(),avgtotalcov[i].end(), greater<uint32>());
      ASI_avgcoverage[i]=avgtotalcov[i][(avgtotalcov[i].end()-avgtotalcov[i].begin())/2];
    }
  }
...
void AssemblyInfo::checkThesePrinter(ostream & ostr, uint32 val, const char * s)
{
  ostr << s << val;
  if(val==0){
    ostr << "\t(excellent)";
...
uint32 AssemblyInfo::huntForSmileCoverage(string & warntext)
{
  ...
  auto thmin=covtotake*.75d;
  auto thmax=covtotake*1.25d;
  ...
  if(sumsize_below >= sumsize_lnr*0.15d){
    ++retvalue;
    wmsg << "- " << num_below << " contig(s) with a total of " << sumsize_below << " bases (= " << 100.0d/sumsize_lnr*sumsize_below << "% of bases in all non-repetitive large contigs) have an average coverage less than 75% of the average coverage of all non-repetitive large contigs.\n";
  }
  // test for proportion of contigs above ...
  if(sumsize_above >= sumsize_lnr*0.15d){
    ++retvalue;
    wmsg << "- " << num_above << " contig(s) with a total of " << sumsize_above << " bases (= " << 100.0d/sumsize_lnr*sumsize_above << "% of bases in all non-repetitive contigs) have an average coverage more than 125% of the average coverage of all non-repetitive large contigs.\n";
  }

  // test for proportion of contigs above ...
  if(sumsize_above+sumsize_below >= sumsize_lnr*0.3d){
    ++retvalue;
    wmsg << "- " << num_above+num_below << " contig(s) with a total of " << sumsize_above+sumsize_below << " bases (= " << 100.0d/sumsize_lnr*(sumsize_above+sumsize_below) << "% of bases in all non-repetitive contigs) have an average coverage 25% above or below the average coverage of all non-repetitive large contigs.\n";
  }
