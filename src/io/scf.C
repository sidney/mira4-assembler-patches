...
int8 SCF::load(const char * givenname, string & emsg)
{
  ...
  if(!fin){
    ...
    for(uint32 ai=0; ai<SCF_suffixalternatives.size(); ai++) {
      string compfn=(static_cast<string>(filename))+SCF_suffixalternatives[ai];
      fin.clear();
      fin.open(compfn.c_str(), ios::in|ios::binary);
      if(fin){
	filefound=true;
	filename=compfn;
	break;
  ...
  if(SCF_header.magic_number!=SCF_MAGIC){
	...
	if(system(command_buffer)){
      ...
      fin.clear();
      fin.open(SCF_tmpfnameforload, ios::in|ios::binary);
      if(!fin){
	discard();
...
void SCF::setBases(const string & newbases, const vector<uint8> & newprobs, const vector<uint32> & newpeakindex)
{
  ...
  if(numbases!=0) {
    ...
    //memcpy(SCF_bases, newbases, numbases*sizeof(uint8));
    {
      ...
      vector<uint32>::const_iterator I=newpeakindex.begin();
	...
	*target=newbases[i];
    ...
    vector<uint8>::const_iterator theprob=newprobs.begin();
...
uint8 SCF::getCalledBaseProb(uint32 index) const
{
  ...
  default:{
    return max(SCF_prob_T[index], max(SCF_prob_G[index], max(SCF_prob_A[index],SCF_prob_C[index])));
  }
  }
}
